...
const numberOfExampleItems = (examples) => (
  (examples || []).length
);
...
const ExamplesSidebar = ({ examples, className }) => {
  ...
  return (
    <Sidebar
      side="right"
      sticky={true}
      className={classnames('hidden xl:block md:w-96 lg:w-3/12 pr-3 self-start sticky top-0', className)}
    >
      <SidebarSection>
      ...
      <SidebarSection>
        ...
        {examples.map((eg) => {
          return (
            <a className="examples-sidebar" href={eg.fields.slug} >
