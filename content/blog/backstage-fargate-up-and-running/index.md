...
Again, we want to write into just some predefined log stream so we should scope the resource section to match that. We can also leave `CreateLogGroup` out in that case since the Fargate task doesn't need permissions to create it.


The last policy we want to create is to allow Fargate download the Docker images we have pushed to our private ECR. 
```json
{
  "Version": "2012-10-17",
  "Statement": [
...
We have our needed environment variables defined and ready to go (make sure you have created a Github token explained [here](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/creating-a-personal-access-token), if you want to use Github as a source). There are few different ways to pass in environment variables to running containers in AWS ECS. We will be using AWS System Manager Parameter Store to save those in a safe place where they can be then loaded to the running container. AWS ECS also provides the possibility to load environment variables from a flat file stored in S3 or pass them in directly (unsecured) to the task definition. 
...
Note that we have not defined the database port to be retrieved from parameter store here. That might be something you want to do if the ports change regularly or are non-standard but not really necessary. 


## Step 4 - Creating a Load Balancer for our Backstage service

The last scaffolding bit we want to do to support our Fargate Backstage is to set up a Load Balancer in standby to wait for our Fargate service to attach itself to it. We do this step a bit prematurely just to have a good static URL available to point to when we eventually start building our application. 
...
We'll continue onwards to setting up our security groups and select create a new one. The only thing we need to listen (in this setup without HTTPS) is to configure this group to allow traffic to port 80 from everywhere (0.0.0.0/0, ::/0). If you want to restrict access to your Backstage instance, you can define and IP range of your offices or VPN, or your personal public IP. 

![lb_sec_groups.png](./lb_sec_groups.png)

For the targets groups we just change the port to 7000, which will be the one our Backstage instance will be using and give the target group a name. We will not be registering any actual targets yet. That will be handled when we spin up our Fargate services. 
...
To deploy the Backstage application we want to have it packaged into a docker image with configurations the best suitable for our environment. We'll start this journey in the Backstage repository. For more information how to scaffold the initial application you can take a look at the post to get [Backstage running with Docker compose](https://roadie.io/blog/backstage-docker-service-catalog/).
For this post we start the same way and scaffold a new fresh Backstage application by running `npx @backstage/create-app`. After we have figured out a good name for the app and selected PostgreSQL as our database provider, we are ready to massage our configuration files to match what we want.

If we take a look at the default `app-config.yaml` file we need to take note of few environment variables that are needed to get the app running properly. These environment variables, for our use case, based on the default `app-config.yaml` file are:
```
POSTGRES_HOST
POSTGRES_PORT
POSTGRES_USER
...
GITHUB_TOKEN
```
These happen to be the same items we created in AWS Parameter Store previously so looks like we are on a good track.
...
We created a Load Balancer to have a more stable DNS entry to be used as the application entrypoint. So let's add that in and modify the `app-config.production.yaml` with to contain that entry. We'll also turn off HTTP->HTTPS redirection from our Content-Security-Policy for now since our Load Balancer only supports port 80. This would be something that should be left as is for more secure environments where HTTPS is set up.
...
We'll scroll down to the environment section where we will add few lines to retrieve our env variables from Parameter store. We need to add keys
```
POSTGRES_HOST
POSTGRES_PORT
POSTGRES_USER
...
And for these the port value is passed in as plain text, other are using the 'ValueFrom' setting and pointing to the ARN of the parameter in Parameter Store.
...
Next we'll create a service that will be spinning up these task definitions. We'll navigate back to the cluster we have created and on the services tab click Create.
...
Next step on wizard is networking configuration. We'll choose our same VPC that our Cluster, RDS and Load Balancer are and select few (or all, our LB supports all of them) of the subnets from the dropdown. We want to assign a public IP to the service in this case because we are accessing a regional AWS service, in this case SSM Paramater Store and we don't have a VPC endpoint set up for it. 
...
Now that our security group allows access from our load balancer, we can click the radio button selection on the Load Balancing section to be Application Load Balancer. We select our load balancer from the drop down, select our container from the second drop down and add that to be balanced. Most of the values are autopopulated for us. We'll create a new target group, that will get automatically assigned to the Load Balancer. We will also change the path patter to be only `/`, at the root of the DNS name. We'll do the same for healt check path.
...
It will take few minutes before it reaches 'RUNNING' status. Unfortunately it doesn't seem to stay in 'RUNNING' status for too long but instead ends up in a loop of starting a new task and failing one after another. 

We can investigate and debug why the running container fails to stay up by checking [CloudWatch logs](https://eu-west-1.console.aws.amazon.com/cloudwatch/home?region=eu-west-1#logsV2:log-groups) that our task has written. In cases where the task doesn't start at all we can take a look at the task itself from the ECS pages to see what prevents it from starting. These could be something like IAM policy issues or perhaps wrong URL to the image that should be started.

When we take a look at the logs we can see that the container starts up and Backstage itself within the container tries it best to start up. It fails on Knex timeout error, telling us that Knex is unable to connect to the database.
...
Setting Backstage up and running on AWS Fargate required multiple steps and configurations but provides a secure and manageable Backstage instance after the initial configuration is done. 
